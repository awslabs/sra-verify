...
class SRA_ORGANIZATIONS_02(OrganizationsCheck):
    ...
    meta = CheckMeta(
        check_id="SRA-ORGANIZATIONS-02",
        title="Organization has foundational OU - Security",
        description=(
        ...
        remediation=Remediation(
            text=(
                "Create an organizational unit named Security directly under the "
                "organization root and move the security accounts into it."
            ),
            cli=(
                "aws organizations create-organizational-unit "
                "--parent-id <root-id> --name Security"
            ),
            console=(
                "AWS Organizations console, AWS accounts, select Root, "
                "Actions, Create new, name the OU Security."
            ),
        ),
    )
    ...
    def execute(self) -> Iterable[Finding]:
        ...
        Yields:
            One Finding for the Security OU.
        """
