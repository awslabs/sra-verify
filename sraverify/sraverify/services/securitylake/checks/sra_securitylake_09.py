...
class SRA_SECURITYLAKE_09(SecurityLakeCheck):
    ...
    meta = CheckMeta(
        check_id="SRA-SECURITYLAKE-09",
        title="Security Lake EKS audit logs enabled with version 2.0 for all organization accounts",
        description=(
        ...
        remediation=Remediation(
            text=(
                "Enable the EKS_AUDIT log source at version 2.0 in Security Lake for "
                "every active organization account and Region."
            ),
            cli=(
                "aws securitylake create-aws-log-source --sources "
                "'[{\"regions\":[\"<region>\"],\"sourceName\":\"EKS_AUDIT\","
                "\"sourceVersion\":\"2.0\"}]' --region <region>"
            ),
            console=(
                "Security Lake console, Settings, Log sources, enable EKS audit logs "
                "at version 2.0."
            ),
        ),
    )
            ...
            if "Error" in log_sources_response:
                ...
                for account_id in active_ids:
                    resource_id = (
                        ...
                        f"log-source/EKS_AUDIT"
                    )
                    if semantic:
                        yield self.failed(
                            region=region,
                            resource_id=resource_id,
                            checked_value="EKS_AUDIT log source configured",
                            actual_value=(
