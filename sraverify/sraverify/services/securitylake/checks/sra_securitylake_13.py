...
class SRA_SECURITYLAKE_13(SecurityLakeCheck):
    ...
    meta = CheckMeta(
        check_id="SRA-SECURITYLAKE-13",
        title="Security Lake VPC flow logs enabled with version 2.0 for all organization accounts",
        description=(
        ...
        remediation=Remediation(
            text=(
                "Enable the VPC_FLOW log source at version 2.0 in Security Lake for "
                "every active organization account and Region."
            ),
            cli=(
                "aws securitylake create-aws-log-source --sources "
                "'[{\"regions\":[\"<region>\"],\"sourceName\":\"VPC_FLOW\","
                "\"sourceVersion\":\"2.0\"}]' --region <region>"
            ),
            console=(
                "Security Lake console, Settings, Log sources, enable VPC Flow Logs "
                "at version 2.0."
            ),
        ),
    )
            ...
            if "Error" in log_sources_response:
                ...
                for account_id in active_ids:
                    resource_id = (
                        ...
                        f"log-source/VPC_FLOW"
                    )
                    if semantic:
                        yield self.failed(
                            region=region,
                            resource_id=resource_id,
                            checked_value="VPC_FLOW log source configured",
                            actual_value=(
